    ...
    for line in f:
        ...
        try:
            ...
            if isinstance(similar_item_html, str):
                obj["similar_item_converted"] = extract_code_from_similar_item(similar_item_html)
                # Extract asins from similar_item_converted
                obj["similar_asins"] = [
                    item.get("asin") for item in obj["similar_item_converted"] if "asin" in item
                ]
            ...
            if isinstance(tech1, str):
                obj["tech1_converted"] = extract_code_from_tech(tech1)
            if isinstance(tech2, str):
                obj["tech2_converted"] = extract_code_from_tech(tech2)
            ...
            if "asin" in obj:
                ...
                if isinstance(also_buy, list):
                    ...
                    obj["similar_asins"].extend(also_buy)
                if isinstance(also_view, list):
                    also_view = [str(asin) for asin in also_view if isinstance(asin, str)]
                    obj["similar_asins"].extend(also_view)

                # Remove duplicates
                obj["similar_asins"] = list(set(obj["similar_asins"]))
